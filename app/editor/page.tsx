...
export default function Home() {
  ...
  return (
    <div className="flex flex-col md:flex-row h-screen w-screen bg-[#f0f2f5]">
      <Sidebar onExecute={executeWorkflow} isExecuting={isExecuting} />

      <div className="flex-1 min-h-[60vh] md:min-h-0" ref={reactFlowWrapper}>
        <ReactFlow
          nodes={nodes}
          edges={edges}
          ...
          defaultEdgeOptions={{
            type: "smoothstep",
            animated: true,
            style: { stroke: "#b1b3b5", strokeWidth: 2 },
          }}
        >
          <Background color="#d1d7db" gap={20} size={1} />
          <Controls className="!bg-white !border-[#d1d7db] !shadow-md" />
          <MiniMap
            nodeColor={(node) => {
              return "#128c7e";
            }}
            className="!bg-white !border-[#d1d7db] !shadow-md"
          ...
          <Panel
            position="top-center"
            className="bg-white px-6 py-2 rounded-full border border-[#d1d7db] shadow-md flex items-center gap-3"
          >
            <div className="text-xs font-bold text-[#128c7e] uppercase tracking-wider">
              Wapzio Editor
            </div>
            <div className="w-px h-4 bg-[#d1d7db]" />
